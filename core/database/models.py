from sqlalchemy import create_engine, Column, Integer, String, Float, Boolean, DateTime, ForeignKey, Enum, func, Numeric, JSON
...
class EncryptedString:
    def __init__(self):
        self.key = config.ENCRYPTION_KEY.encode() if hasattr(config, 'ENCRYPTION_KEY') else Fernet.generate_key()
...
class Wallet(Base):
    ...
    _private_key = Column('private_key', String)
    balance = Column(Float, default=0.0)
    ...
    def private_key(self) -> str:
        return encrypted_string.decrypt(self._private_key) if self._private_key else None

    @private_key.setter
    def private_key(self, value: str):
        self._private_key = encrypted_string.encrypt(value) if value else None
...
class Order(Base):
    ...
    from_token = Column(String(50), nullable=False)
    to_token = Column(String(50), nullable=False)
...
class SystemMetric(Base):
    ...
    memory_usage = Column(Numeric(precision=5, scale=2))
    disk_usage = Column(Numeric(precision=5, scale=2))
    network_in = Column(Numeric(precision=20, scale=2))
    network_out = Column(Numeric(precision=20, scale=2))
