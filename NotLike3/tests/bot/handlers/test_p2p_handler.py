...
from bot.handlers.p2p_handler import (
    ...
    confirm_payment_handler, P2POrderStates
)
...
class TestP2PHandler(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.callback_query_mock.from_user.username = "testcallbackuser" #  callback
    ...
    async def test_confirm_p2p_order_success(self):
        ...
        self.state_mock.get_data.return_value = {
            'side': 'BUY', 'base_currency': 'TON', 'quote_currency': 'USDT',
    ...
    async def test_confirm_p2p_order_invalid_input(self):
    ...
    @patch('bot.handlers.p2p_handler.p2p_order_keyboard')
    async def test_take_p2p_order_handler_success(self, mock_keyboard):
        ...
        self.p2p_service_mock.get_order_by_id.return_value = P2POrder(user_id=456)  #  get_order_by_id
        ...
        self.callback_query_mock.message.bot.send_message.assert_awaited_once_with(456, "Ваш P2P ордер #1 был принят пользователем @testcallbackuser!")  #  уведомление
    ...
    async def test_take_p2p_order_handler_failure(self):
        ...
        self.callback_query_mock.answer.assert_called_once()
        self.callback_query_mock.message.bot.send_message.assert_not_awaited()  #  уведомлений

    @patch('bot.handlers.p2p_handler.p2p_order_keyboard')
    async def test_cancel_p2p_order_handler_success(self, mock_keyboard):
        """Тест cancel_p2p_order_handler (успех)."""
        self.callback_query_mock.data = "p2p_cancel_1"
        self.p2p_service_mock.cancel_order.return_value = {'success': True}
        self.p2p_service_mock.get_order_by_id.return_value = P2POrder(taker_id=456)  # get_order_by_id
        ...
        self.callback_query_mock.answer.assert_called_once()
        self.callback_query_mock.message.bot.send_message.assert_awaited_once_with(456, "P2P ордер #1 был отменен пользователем @testcallbackuser.")  #  уведомление
    ...
    async def test_confirm_payment_handler_success(self):
        ...
        self.callback_query_mock.answer.assert_called_once()
        self.callback_query_mock.message.bot.send_message.assert_awaited_once_with(456, "Пользователь @testcallbackuser подтвердил получение оплаты по ордеру #1.")  #  уведомление
    ...
    async def test_confirm_payment_handler_wrong_user(self):
        """Тест confirm_payment_handler (неверный пользователь)."""
        self.callback_query_mock.data = "p2p_confirm_payment_1"
        order = P2POrder(id=1, user_id=456, taker_id=123, status=P2POrderStatus.IN_PROGRESS)  #  user_id
        self.p2p_service_mock.get_order_by_id.return_value = order
        await confirm_payment_handler(self.callback_query_mock, self.state_mock, self.p2p_service_mock)
        self.callback_query_mock.message.answer.assert_called_with("Вы не можете подтвердить оплату для этого ордера.")
        self.callback_query_mock.answer.assert_called_once()
        self.p2p_service_mock.complete_order.assert_not_awaited()
    ...
    async def test_confirm_payment_handler_wrong_status(self):
        ...
        self.callback_query_mock.data = "p2p_confirm_payment_1"
        order = P2POrder(id=1, user_id=123, taker_id=456, status=P2POrderStatus.OPEN)  #  status
        self.p2p_service_mock.get_order_by_id.return_value = order
        await confirm_payment_handler(self.callback_query_mock, self.state_mock, self.p2p_service_mock)
    ...
    async def test_confirm_payment_handler_failure(self):
        """Тест confirm_payment_handler (ошибка)."""
        self.callback_query_mock.data = "p2p_confirm_payment_1"
        order = P2POrder(id=1, user_id=123, taker_id=456, status=P2POrderStatus.IN_PROGRESS)
        self.p2p_service_mock.get_order_by_id.return_value = order
